...
def gibson():
    ...
    for i in range(1, int(insert_number)):
        name['insert'+str(i)] = c1.text_input(f'Insert {str(i)}', key=str(i+1), value='insert '+str(i))
        length['insert'+str(i)] = c2.number_input('Lenght (bp)', key=str(i+1), value=100, min_value=0)
        concentration['insert'+str(i)] = c3.number_input('Concentration (ng/ul)', key='lol'+str(i+1), value=1.0, min_value=0.0)
        ratio['insert'+str(i)] = c4.number_input('Insert:vector ratio', key='lol'+str(i+1), value=3.0, min_value=0.0)
